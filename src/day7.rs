...
fn parse_bag_quantity(line : &str, re_bag_quantity : &regex::Regex) -> Vec<BagQuantity> {
    ...
    for l in line.split(',') {
        ...
        result.push(BagQuantity {name : m[2].to_string(), nr : m[1].parse().unwrap()} );
    }
    return result;
}
...
fn parse_bag(line : &str, re_bag : &regex::Regex, re_bag_quantity : &regex::Regex) -> Bag {
    ...
    return Bag { name : m[1].to_string() , bags: parse_bag_quantity(&m[2], re_bag_quantity)}
}
...
pub fn day7() {
    ...
    println!("Day 7, part 2: {}", contains_bags(&bags["shiny gold"], &bags) - 1);
}