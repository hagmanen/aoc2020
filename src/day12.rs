...
fn parse_instructions(lines : std::str::Lines) -> Vec<Instruction> {
  ...
  for line in lines {
    ...
    result.push(Instruction {op : m[1].chars().next().unwrap(), arg : m[2].parse().unwrap()});
  }
  return result;
}
...
fn step1(instruction : &Instruction, x : &mut i32, y : &mut i32, dir : &mut i32) {
  match instruction.op {
    ...
    'E' => *x += instruction.arg,
    'W' => *x -= instruction.arg,
    'L' => {*dir += instruction.arg; *dir %= 360;},
    'R' => {*dir += 360 - instruction.arg; *dir %= 360;},
...
fn step2(instruction : &Instruction, pos_x : &mut i32, pos_y : &mut i32, way_x : &mut i32, way_y : &mut i32) {
  match instruction.op {
    ...
    'F' => {*pos_x += instruction.arg * (*way_x); *pos_y += instruction.arg * (*way_y);},
    _ => panic!("Unknown operation"),
  }
}
